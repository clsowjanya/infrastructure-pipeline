		...
		stage('Deploy DB') {
			steps {
				...
				withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', accessKeyVariable: 'AWS_ACCESS_KEY_ID', credentialsId: 'jenkins-aws', secretKeyVariable: 'AWS_SECRET_ACCESS_KEY']]) {
					sh("aws cloudformation create-stack --region us-east-1 --stack-name dynacorp-database --template-body file://https://github.com/clsowjanya/infrastructure-pipeline/blob/master/dynamodb.yaml")
				}
			}
 	    }
